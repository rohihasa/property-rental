...
function UsersPageForAdmin() {
  ...
  const handleApproveOrReject = (action, userId) => {
    UserService.approveOrRejectOwner(action, userId)
      .then((response) => {
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    {user.roles.includes("ROLE_OWNER") &&
                      (user.verified ? (
                        <Button
                          onClick={() => handleApproveOrReject("reject",user.id)}
                          variant="contained"
                          color="secondary"
                        >
                          Revoke Owner
                        ...
                        <>
                          <Button
                            onClick={() =>
                              handleApproveOrReject("approve", user.id)
                            }
                            style={{ margin: "12px" }}
                            variant="contained"
                            color="primary"
                          ...
                          <Button
                            onClick={() =>
                              handleApproveOrReject("reject", user.id)
                            }
                            variant="contained"
                            color="secondary"
                          >
