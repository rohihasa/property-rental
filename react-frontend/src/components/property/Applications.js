...
function Applications() {
  ...
  const handleDownload = (type,id) => {
    if(type==="credit"){
        UserService.viewOrDownloadAttachment("credit",id,false)
    }else{
        UserService.viewOrDownloadAttachment("id",id,false)
    }
  ...
  const handleView = (type,id) => {
    if(type==="credit"){
        UserService.viewOrDownloadAttachment("credit",id,true)
    }else{
        UserService.viewOrDownloadAttachment("id",id,true)
  ...
  const handleReject = (applicationId) => {
    console.log("Reject");
    UserService.acceptOrRejectApplicationByOwner(applicationId, "REJECT")
      .then((response) => {
        console.log("Response::", response.data);
          ...
          <Box p={2} m={1} style={{ border: "1px solid black" }}>
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <Box p={1}>
                  ...
                  <div className="attachment-card-1">
                    ...
                    <div>
                      <button
                        onClick={()=>handleDownload("credit",application.id)}
                        className="attachment-button-1 download-button-1"
                      >
                        Download
                      </button>
                      <button
                        onClick={()=>handleView("credit",application.id)}
              ...
              <div className="attachment-card-1">
                    ...
                    <div>
                      <button
                        onClick={()=>handleDownload("id",application.id)}
                        className="attachment-button-1 download-button-1"
                      >
                        Download
                      </button>
                      <button
                        onClick={()=>handleView("id",application.id)}
                        className="attachment-button-1 view-button-1"
                      >
                        View
                      </button>
